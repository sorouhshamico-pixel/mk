    ...
    <>
      ...
      <Reveal>
        <section className="mx-auto max-w-[900px] px-6 pb-16 sm:pb-24">
          <div className="flex flex-col items-start gap-4 border-s-2 border-accent ps-6">
            <Quote size={28} className="text-accent" />
            <p className="text-xl leading-[1.5] text-ink sm:text-2xl">{about.pullQuote}</p>
          </div>
        </section>
      </Reveal>
      ...
      <section className="mx-auto flex max-w-[900px] flex-col gap-10 px-6 pb-16 sm:pb-24">
        <Reveal>
          <h2 className="text-[11px] uppercase tracking-[0.14em] text-muted">How I got here.</h2>
        </Reveal>
        <div className="flex flex-col gap-10">
          {about.story.map((paragraph, i) => (
            <Reveal key={paragraph} delay={i * 80}>
              <div className="grid grid-cols-[80px_1fr] gap-4 sm:grid-cols-[110px_1fr] sm:gap-8">
                <span className="pt-1 font-mono text-xs text-muted">{storyEras[i]}</span>
                <p className="text-[15px] leading-[1.8] text-secondary sm:text-base">{paragraph}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="mx-auto flex max-w-[1200px] flex-col gap-10 px-6 pb-16 sm:pb-24">
        <Reveal>
          <div className="flex flex-col gap-2">
            <h2 className="text-[11px] uppercase tracking-[0.14em] text-muted">How I work.</h2>
            <p className="text-secondary">Four working principles, not aspirational copy.</p>
          </div>
        </Reveal>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ol className="flex flex-col gap-5 border-s-[0.5px] border-hairline">
              {about.experience.map((e) => (
                <li key={e.org} className="relative flex flex-col gap-0.5 ps-5">
                  <span
                    aria-hidden="true"
                    className="absolute -start-[3px] top-1.5 h-[5px] w-[5px] rounded-full bg-accent"
                  />
          ...
          <div className="flex flex-col gap-4">
            ...
            <ol className="flex flex-col gap-5 border-s-[0.5px] border-hairline">
              {about.education.map((e) => (
                <li key={e.title} className="relative flex flex-col gap-0.5 ps-5">
                  <span
                    aria-hidden="true"
                    className="absolute -start-[3px] top-1.5 h-[5px] w-[5px] rounded-full bg-accent"
                  />
      ...
      <Reveal>
        <section className="mx-auto max-w-[1200px] px-6 pb-24">
          <div className="flex flex-col gap-6 rounded-2xl border-[0.5px] border-hairline bg-gradient-to-br from-surface to-paper p-8 shadow-lg">
            <h2 className="text-[11px] uppercase tracking-[0.14em] text-muted">Beyond the screen.</h2>
